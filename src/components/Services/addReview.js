...
import profile from "../../assets/images/bussiness-man.jpg";
...
const ReviewList = () => {
  ...
  const handleUpdateReview = async () => {
    ...
    await axios.put(
      `http://localhost:8081/api/reviews/${editingReview.id}`,
      editingReview
    );
    fetchReviews();
    setReviews(
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-purple-100 p-8">
      ...
      <div className="max-w-3xl mx-auto p-4 mt-32">
        ...
        {reviews.map((review) => (
          <div key={review.id} className="mb-4 p-4 rounded-2xl bg-white  ">
            <img
              src={profile}
              alt="profile"
              className="size-32 float-start flex mr-11"
            />
            <h2 className="text-2xl font-bold">{review.foodName}</h2>
            <div className="flex items-center">
              <StarRating rating={review.rating} onRatingChange={() => {}} />
            </div>
            <p className="ml-2 text-2xl">{review.restaurantName}</p>
